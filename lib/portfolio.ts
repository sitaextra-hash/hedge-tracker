...
export function computeHoldings(
  ...
  const computed = holdings.map((h) => {
    ...
    return { ...h, currentPrice, previousClose: price?.previousClose ?? currentPrice, shares, currentAmount, priceSource: price?.source ?? ("unavailable" as const), currentPct: 0, drift: 0, dayChangePct: 0 };
  });
